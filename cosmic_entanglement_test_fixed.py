...
class CosmicEntanglementTest:
    ...
    def visualize_results(self):
        ...
        violations = bell_data['violations']
        plt.bar(range(len(violations)), violations)
        plt.axhline(y=0, color='r', linestyle='-')
        plt.title('Bell Inequality Violations by Region Pair')
        ...
        plt.subplot(2, 2, 2)
        phi_corr = self.results['phi_correlation']
        angles = phi_corr['angles']
        correlations = phi_corr['correlations']
        plt.plot(angles, correlations)
        # Mark phi-related angles with vertical lines
        for i in range(1, 6):
            angle = 180 / (self.phi ** i)
            plt.axvline(x=angle, color='g', linestyle='--', alpha=0.5, 
                        label='Phi-related angle' if i == 1 else None)
        plt.title('Angular Correlations')
        ...
        plt.axvline(x=z, color='r', linewidth=2, 
                  label='Z-score: {:.2f}'.format(z))
        plt.axvline(x=1.96, color='g', linestyle='--', label='p=0.05 threshold')
        ...
        gauge = np.linspace(-1, 1, 100)
        plt.barh(0, phi_opt, height=0.5, color='b' if phi_opt >= 0 else 'r')
        plt.xlim(-1, 1)
        plt.axvline(x=0, color='k', linestyle='-')
        plt.title('Phi Optimality: {:.4f}'.format(phi_opt))
        plt.xlabel('Phi Optimality (-1 to 1)')
        plt.yticks([])
        
        # Add overall title
        plt.suptitle('Cosmic Entanglement Test Results', fontsize=16)
    ...
    def plot_phi_optimization_gauge(self, ax):
        """Plot the phi optimization gauge."""
        phi_optimality = self.results['phi_optimality']
        
        # Create a gauge-like visualization
        gauge = np.linspace(-1, 1, 100)
        ax.barh(0, phi_optimality, height=0.5, color='b' if phi_optimality >= 0 else 'r')
        ax.set_xlim(-1, 1)
        ax.axvline(x=0, color='k', linestyle='-')
        ax.set_title('Phi Optimality: {:.4f}'.format(phi_optimality))
        ax.set_xlabel('Phi Optimality (-1 to 1)')
        ax.set_yticks([])
        ...
        if phi_optimality > 0.5:
            ...
            interpretation = "MODERATE anti-phi-optimization"
        else:
            interpretation = "STRONG anti-phi-optimization"
            
        ax.text(0, -0.5, interpretation, 
